...
*/
$tglBulan = $db['tgl_absen'];
if ($tglBulan !== null && strtotime($tglBulan) !== false) {
    $tglTerakhir = date('t', strtotime($tglBulan));
} else {
    // Handle invalid date format or null value
    // For example:
    $tglTerakhir = ''; // Set a default value
    // Or output an error message:
    // echo "Error: Invalid date format or tgl_absen is null.";
}
...
<tr>
    ...
    <td rowspan="2" bgcolor="#EFEBE9" align="center">L/P</td>
    <?php
    // Ensure $tglBulan is not null and is a valid date string
    $year = ''; // Initialize year variable
    if ($tglBulan !== null && strtotime($tglBulan) !== false) {
        $year = date('Y', strtotime($tglBulan));
    }
    ?>
    <td colspan="<?= $tglTerakhir; ?>" style="padding: 8px;">
        PERTEMUAN BULAN : <b style="text-transform: uppercase;"><?php echo namaBulan($bulan) . ' ' . $year; ?></b>
    </td>
    <td colspan="5" align="center" bgcolor="#EFEBE9">JUMLAH</td>
</tr>
